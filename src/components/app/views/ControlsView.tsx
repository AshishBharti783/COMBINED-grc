...
export function ControlsView() {
  ...
  useEffect(() => {
    api('/api/frameworks').then((d: any) => {
      ...
      // If a framework was pre-selected via deep-link, use it; otherwise default.
      const fw = saved ? d.frameworks.find((f: any) => f.id === saved) : null
      if (fw) setSelectedFramework(fw.id)
      else if (d.frameworks[0]) setSelectedFramework(d.frameworks[0].id)
      // Deep-link status filter from dashboard pie chart click.
      const savedStatus = sessionStorage.getItem('controlStatusFilter')
      if (savedStatus) {
        setStatusFilter(savedStatus)
        sessionStorage.removeItem('controlStatusFilter')
      }
      // Clean up framework hint after consumption so reloads don't re-apply.
  ...
  // Download a blank CSV template with framework-specific headers + example rows.
  // The user fills this in and uploads it via the Import dialog.
  async function downloadTemplate(framework: string) {
    try {
      const res = await fetch(`/api/controls/template?framework=${framework}`, { cache: 'no-store' })
      if (!res.ok) throw new Error('Failed to download template')
      const blob = await res.blob()
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      // Extract filename from Content-Disposition header, fallback to a sensible name
      const cd = res.headers.get('Content-Disposition') || ''
      const match = cd.match(/filename="([^"]+)"/)
      a.download = match ? match[1] : `${framework.toLowerCase()}_controls_template.csv`
      a.click()
      URL.revokeObjectURL(url)
      toast.success(`Downloaded ${framework} CSV template`)
    } catch (e: any) {
  ...
  return (
    <div>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline">
                    <FileText className="w-4 h-4 mr-2" /> CSV Template
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem onClick={() => downloadTemplate('ISO27001')}>
                    ISO 27001 format
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => downloadTemplate('SOC2')}>
                    SOC 2 format
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => downloadTemplate('GDPR')}>
                    GDPR format
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => downloadTemplate('HIPAA')}>
                    HIPAA format
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => downloadTemplate('PCI_DSS')}>
                    PCI DSS format
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => downloadTemplate('NIST_CSF')}>
                    NIST CSF format
                  </DropdownMenuItem>
                  <div className="h-px bg-border my-1" />
                  <DropdownMenuItem onClick={() => downloadTemplate('generic')}>
                    Generic format
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <Card><CardContent className="p-3 h-full"><div className="text-xs text-muted-foreground">Total</div><div className="text-xl font-bold">{stats.total}</div></CardContent></Card>
        <Card><CardContent className="p-3 h-full"><div className="text-xs text-muted-foreground">Compliant</div><div className="text-xl font-bold text-emerald-600">{stats.compliant}</div></CardContent></Card>
        <Card><CardContent className="p-3 h-full"><div className="text-xs text-muted-foreground">In Progress</div><div className="text-xl font-bold text-amber-600">{stats.inProgress}</div></CardContent></Card>
        <Card><CardContent className="p-3 h-full"><div className="text-xs text-muted-foreground">Not Started</div><div className="text-xl font-bold text-slate-500">{stats.notStarted}</div></CardContent></Card>
      </div>
            ...
            <Card key={c.id} className="overflow-hidden">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-14 h-12 rounded-lg bg-primary/10 flex items-center justify-center shrink-0 px-1">
                    <span className="text-[10px] font-bold text-primary leading-tight text-center break-all line-clamp-2">{c.ref}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                      ...
                      {c.assignment?.owner && <span className="text-[11px] text-muted-foreground">· {c.assignment.owner}</span>}
                    </div>
                  </div>
