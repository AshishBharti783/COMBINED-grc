...
export function EvidenceView() {
  ...
  useEffect(() => {
    ...
    // Deep-link search from dashboard click.
    const savedSearch = sessionStorage.getItem('evidenceSearch')
    if (savedSearch) {
      setSearch(savedSearch)
      sessionStorage.removeItem('evidenceSearch')
    }
  }, [isSuperAdmin])
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        <StatCard label="Total Evidence" value={stats.total} icon={FolderOpen} />
        <StatCard label="Files" value={stats.files} icon={FileText} tone="warning" />
        <StatCard label="Links" value={stats.links} icon={Link2} tone="info" />
                          ...
                          <DropdownMenuContent align="end">
                            {e.type === 'file' && e.filePath && (
                              <DropdownMenuItem asChild>
                                <a href={`/api/evidence?download=${e.id}`} download={e.fileName || undefined} className="cursor-pointer">
