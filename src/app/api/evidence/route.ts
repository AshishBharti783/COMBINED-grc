...
export async function GET(req: NextRequest) {
  ...
  if (downloadId) {
    try {
      const record = await db.evidence.findUnique({ where: { id: downloadId } })
      if (!record || record.type !== 'file' || !record.filePath) {
        return NextResponse.json({ error: 'File not found' }, { status: 404 })
      }
      if (!canAccessTenant(user, record.tenantId)) {
        return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
      }

      // Try public/uploads path first, then try relative to cwd
      let fullPath = path.join(process.cwd(), 'public', record.filePath)
      try {
        await stat(fullPath)
      } catch {
        // Fallback: maybe filePath is already absolute or relative to cwd
        fullPath = path.join(process.cwd(), record.filePath.replace(/^\//, ''))
        try {
          await stat(fullPath)
        } catch {
      ...
      headers.set('Content-Disposition', `attachment; filename="${(record.fileName || 'download').replace(/"/g, '\\"')}"`)
      ...
      return NextResponse.json({ error: 'File not found on disk. It may not have been uploaded properly.' }, { status: 404 })
    }
  }
...
export async function PATCH(req: NextRequest) {
  ...
  const body = await req.json()
  const { id: bodyId, title, description, status, tags, fileUrl, linkTitle } = body
